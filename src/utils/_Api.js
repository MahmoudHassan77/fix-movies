...
export const getAllMovies = (page,type) => {
    ...
    return new Promise((res, rej) => {
      axios({
        method: "get",
        url: `${url}movie/${type}?api_key=${apiKey}&language=en-US&page=${page}`,
        headers,
      })
        .then((response) => {
          res(response);
  ...
  export const searchAboutMovie = (text,type) => {
    const headers = {
      "Content-Type": "application/json",
    };
    headers.Authorization = `Bearer ${token}`;
    ...
    return new Promise((res, rej) => {
      axios({
        method: "get",
        url: `${url}search/movie?api_key=${apiKey}&language=en-US&query=${text}`,
        headers,
      })
        .then((response) => {
          res(response);
